  ...
  return (
    <div className="cmd-menu">
      ...
      {filtered.map((cmd, i) => (
        ...
        >
          {/* color dynamique par commande → inline */}
          <span className="cmd-menu-id" style={{ color: cmd.color }}>{cmd.id}</span>
          <span className="cmd-menu-desc">{cmd.desc}</span>
          <span
            className="cmd-menu-badge"
            style={{ background: `${cmd.color}18`, color: cmd.color, border: `1px solid ${cmd.color}33` }}
          >
            {cmd.context.slice(1)}
          </span>
        </button>
...
function AIContent() {
  ...
  // ── Couleurs / labels par rôle → inline (dynamiques) ────────
      ...
      <div className="ai-agents">
        {[
          ...
          <div key={a.name} className="ai-agent">
            {/* couleur + animation dépendent du streaming → inline */}
            <span
              className="ai-agent-dot"
              style={{ color: a.color, animation: a.status === 'THINKING' ? 'pulse 1s infinite' : 'none' }}
            >●</span>
        ...
        <div className="ai-cmd-hints">
          {COMMANDS.slice(0, 3).map(c => (
            <button
              ...
              className="ai-cmd-hint"
              style={{ color: c.color, border: `1px solid ${c.color}33` }}
            >
              {c.id}
            </button>
          ))}
      ...
      >
        {messages.map((m, i) => {
                ...
                <div className="ai-action-card">
                  ...
                  <div className="ai-action-params">
                    {Object.entries(parsed.params).map(([k, v]) => (
                      <div key={k}>
                        <span className="ai-action-key">{k}: </span>
                        <span className="ai-action-val">{v}</span>
                      </div>
                    ))}
                  </div>
                  {isLast && pendingAction && (
                    <div className="ai-action-btns">
                      ...
                      <button onClick={handleCancelAction} className="ai-action-btn ai-action-btn--cancel">
                        CANCEL
                      </button>
                    </div>
                  )}
          ...
          return (
            <div key={i} className="ai-msg">
              <span className="ai-msg-ts">{fmtTime(m.ts)}</span>
              {/* couleur dépend du rôle → inline */}
              <span className="ai-msg-role" style={{ color: roleColor[m.role] }}>
                {roleLabel[m.role]}&gt;
              </span>
              <span
