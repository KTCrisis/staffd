...
drop function if exists ebitda_monthly(uuid, date, date, date)        cascade;
drop function if exists working_days(date, date)                      cascade;
...
drop function if exists issue_invoice(uuid)                           cascade;
drop function if exists invoices_guard()                              cascade;
drop function if exists invoice_lines_guard()                         cascade;
...
create table if not exists clients (
  ...
  -- Mentions de facture (0010)
  billing_address text, siren text, tva_number text,
...
-- Charges d'exploitation (0008, EBITDA), admin seulement
...
create table if not exists consultants (
  ...
  -- Présence et honoraires (0008, EBITDA) : mois comptés dans la masse
  -- salariale ; forfait HT d'un associé facturé via sa société
  date_entree date,
  date_sortie date,
...
create table if not exists invoices (
  ...
  invoice_number       text,            -- attribué à l'émission (issue_invoice), NULL en brouillon
...
-- ── Factures (0010) : émission numérotée et verrou ─────────────────────────
...
-- operating_expenses (0008) : admin seulement
